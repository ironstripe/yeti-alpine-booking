ALTER TABLE public.tickets
  ...
  ADD COLUMN IF NOT EXISTS reservation_token text DEFAULT encode(gen_random_bytes(16), 'hex');
...
DROP TRIGGER IF EXISTS trg_generate_customer_number ON public.customers;
...
DECLARE
  r RECORD;
BEGIN
  FOR r IN SELECT id FROM public.customers WHERE customer_number IS NULL ORDER BY created_at LOOP
    UPDATE public.customers
    SET customer_number = 'KD-' || LPAD(nextval('public.customer_number_seq')::text, 6, '0')
    WHERE id = r.id;
  END LOOP;
END;
$do$;
    ...
    FOR v_inst IN
      SELECT i.id,
        (SELECT count(*) FROM public.ticket_items ti
         ...
         WHERE ti.instructor_id = v_inst.id AND ti.date = v_date
    ...
    LOOP
      ...
      IF NOT v_conflict AND EXISTS (
        ...
        WHERE a.instructor_id = v_inst.id
          ...
          AND (a.is_full_day OR (a.time_start < v_end AND a.time_end > v_start))
      ) THEN v_conflict := true; END IF;
